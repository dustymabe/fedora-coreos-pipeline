...
def cloud_testing_enabled_for_arch(pipecfg, cloud, basearch) {
    if (pipecfg.clouds."${cloud}"?.test_architectures) {
        // The list exists. Return true if the arch is in the list.
        if (basearch in pipecfg.clouds."${cloud}".test_architectures) {
            return true
        } else {
            return false
        }
...
// Runs followup could tests based on conditions
