...
try { timeout(time: 120, unit: 'MINUTES') {
    ...
    if (currentBuild.result != 'SUCCESS') {
        slackSend(color: 'danger', message: ":fcos: :trashfire: kola-openstack <${env.BUILD_URL}|#${env.BUILD_NUMBER}> [${params.STREAM}][params.ARCH]")
    }
}
