...
lock(resource: "build-${params.STREAM}-${params.ARCH}", extra: [[resource: "release-${params.VERSION}-${params.ARCH}"]]) {
    ...
    node(pod_label) { container('coreos-assembler') {
        ...
        stage('Init Remote') {
            ...
            withPodmanRemote {
                container = shwrapCapture("cosa remote-session create --image ${image}")
            }

            withEnv COSA_REMOTE_CONTAINER=${container} {
                shwrap("""
                cosa init --force --branch ${ref} --commit=${fcos_config_commit} ${src_config_url}
                """)
            }
        ...
        stage('Fetch') {
            if (s3_stream_dir) {
                ...
                shwrap("""
                cosa remote-session rsync -avh ${local_builddir} :${local_builddir}
        ...
        if (basearch =="aarch64" && uploading && !is_mechanical) {
            parallelruns['Upload AWS'] = {
                XXX copy AWS_FCOS_BUILDS_BOT_CONFIG to remote container
        ...
        stage('Archive') {
            ...
            if (uploading) {
              ...
              cosa remote-session rsync -avh :builds/ ${local_builddir}
              """)
            }
        }

        // XXX break out of remote session
        ...
        stage('Sync Data') {
            ...
            cosa remote-session rsync :${meta_json} ${meta_json}
            cosa remote-session rsync :builds/builds.json builds/builds.json
